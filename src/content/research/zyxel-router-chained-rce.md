---
title: Zyxel Router Chained RCE via LFI and Weak Password Derivation
topicName: Zyxel router chained RCE using LFI and Weak Password Derivation Algorithm (No CVE)
cve: null
description: Multiple Zyxel devices are prone to different critical vulnerabilities resulting from insecure coding practices and insecure configuration.
published: '2023-04-14'
revised: '2023-04-28'
disclosureDate: null
source:
  platform: Rapid7 AttackerKB
  url: https://attackerkb.com/topics/f567cbf9-5873-48f3-a01f-dd1a0d02eb9f
...
exploitArtifacts:
- type: metasploit
  label: 'exploits/linux/http/zyxel_lfi_unauth_ssh_rce'
  officialUrl: https://github.com/rapid7/metasploit-framework/blob/master/modules/exploits/linux/http/zyxel_lfi_unauth_ssh_rce.rb
  pullRequestUrl: https://github.com/rapid7/metasploit-framework/pull/17881
references:
- label: 'Zyxel Advisory'
...
In December 2022, `SEC Consult` released a blog with the title [The enemy from within: Unauthenticated Buffer Overflows in Zyxel routers still haunting users ](https://sec-consult.com/blog/detail/enemy-within-unauthenticated-buffer-overflows-zyxel-routers/). The blog explains an unauthenticated buffer overflow in more than 40 different Zyxel router models and many thousands of routers that are vulnerable and accessible via the Internet.
The impact of that specific exploit path is limited because the published [Metasploit exploit module](https://github.com/rapid7/metasploit-framework/pull/17388) only works from the LAN side.
...
While reviewing the advisory and the additional vulnerabilities, I identified an exploitation path that chains two separate issues that will allow an unauthenticated attacker to get privileged access to the Zyxel router from either the WAN or LAN side. Because the chain can be reached from the WAN side on affected configurations, its practical impact is higher than the LAN-only exploitation path.

Recently, [Related research: CVE-2023-28770](/research/cve-2023-28770/) has been released covering the LFI vulnerability that is used in this chained exploit.
...
```
The information disclosure alone does not immediately compromise the router. Offline cracking of the retrieved password hashes is possible, but may be impractical depending on password strength.

So is there any other way to use the disclosed information for a successful attack?
The answer is `YES`: the disclosed device information can be combined with the password-derivation weakness described below.
...
- [Related research: CVE-2023-28770](/research/cve-2023-28770/)
